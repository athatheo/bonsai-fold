...
Training-free structural compression of a 1-bit 27B LLM, measured end-to-end on a single 64 GB Apple Silicon laptop. No training, no fine-tuning, no calibration-based updates to surviving weights; every claim is gated by a 500-item, four-task evaluation run.
...
Model packs are not committed (multi-GB); the base checkpoints are public on Hugging Face (`prism-ml/Bonsai-27B-mlx-1bit`, `prism-ml/Bonsai-8B-mlx-1bit`). Setup:
