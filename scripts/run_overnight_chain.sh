...
git add experiments/minibench/results/k4.json
git commit -q -m "minibench: k4 complete (fit-form discriminator)" || true
...
if [ -f "$COMPARATOR/config.json" ] && ls "$COMPARATOR"/*.safetensors >/dev/null 2>&1; then
  ...
  git add experiments/minibench/results/external_qwen3.5-4b-8bit.json
  git commit -q -m "minibench: external comparator Qwen3.5-4B-8bit complete" || true
else
